      ...
      <div class="container">
        ...
        <div class="row">
          <div class="col-lg-8">
            ...
            <div class="mb-5">
              <h3 class="h5 d-flex align-items-center mb-4 text-primary"><span class="icon-rocket mr-3"></span>Responsabilites</h3>
              <p> {{$job->responsibilities}} </p>
            </div>

            <div class="mb-5">
              <h3 class="h5 d-flex align-items-center mb-4 text-primary"><span class="icon-book mr-3"></span>Education_Experience</h3>
            ...
            <div class="row mb-5">
              <div class="col-6">
                ...
                <form action="{{ route('save.job') }}" methode="POST">
                  @csrf 
              ...
              <div class="col-6">
              <form action="{{ route('apply.job') }}" methode="POST">
                  @csrf 
                  <input name="job_id" type="hidden" value="{{ $job->id }}"> 
                  <input name="job_image" type="hidden" value="{{ $job->image }}">
                  <input name="job_title" type="hidden" value="{{ $job->job_title }}"> 
                  ...
                  @if(isset(Auth::user()->id))
                      ...
                      @if ($appliedJob > 0)
                          <button class="btn btn-block btn-success btn-md"disabled>You Applied This Job</button>
                      @else 
                          <button name="submit" type="submit" class="btn btn-block btn-light btn-md">Apply Job</button>
                      @endif

                  @else 
                           <a href="{{ route('login') }}" class="btn btn-block btn-success btn-md">Login to apply for this job</a>
                  @endif
              </form>
              </div>
            </div>
          ...
          <div class="col-lg-4">
            ...
            <div class="bg-light p-3 border rounded">
              ...
              <div class="px-3">
                ...
                <a href="https://twitter.com/intent/tweet?text={{ $job->title }}&url={{ route('single.job' , $job->id) }}" class="pt-3 pb-3 pr-3 pl-0"><span class="icon-twitter"></span></a>
